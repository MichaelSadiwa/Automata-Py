...
defaults = {
    ...
    "disabled": True,
    "placeholder_text": "",
    "regex_input": utils.regex_options[0],
    "selected_pattern": "",
...
# Only run logic if expression selected
if regex_input != utils.regex_options[0]:
    ...
    if st.session_state.trigger_validation and string_input.strip():
        input_clean = string_input.strip()
        if not all(char in current_dfa["alphabet"] for char in input_clean):
            st.session_state.invalid_inputs.append(input_clean)
            st.error(f"Invalid characters! Allowed: {current_dfa['alphabet']}")
        else:
            st.write(f"Analyzing: `{input_clean}`")
            with st.spinner("🔄 The program is tracing..."):
                is_valid, checks, steps = utils.validate_dfa(current_dfa, input_clean)
                utils.animate_dfa_validation(current_dfa, checks, steps)
            if is_valid:
                st.session_state.valid_inputs.append(input_clean)
